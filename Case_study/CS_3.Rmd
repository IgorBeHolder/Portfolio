...
2.  Annual weekend membership program - any number of rides totally up to 60 min on Saturdays and 60 min on Sundays included for 150\$ a year. (April - September - 48 days means only 3\$ on e-bike a day).

3.  Customers may consider to buy personal e-bikes, but we should launch **an awareness campaign** about the benefits of e-bike sharing (charging, service, does not take up space in the apartment, not have to worry a single second about a bike being stolen, etc.)

4.  Investigate most popular casual riders routes in Chicago and inform potential customers about future locations of dock stations. Conduct a survey among potential customers where to locate stations.
...
1.  Add Chicago 2022 weather (temperature, precipitation)
...
skim_without_charts(w_df)
```

```{r casting w_df dates}
# casting date
w_df$weather_date = mdy_hms(w_df$Measurement.Timestamp)
# trans temp Air Temperature in Celsius degrees.
# w_df$TempC <- 5/9*(w_df$Air.Temperature - 32)
```

```{r}
w_df %>% 
  filter(weather_date >= "2022-01-01" & weather_date <= "2023-01-30") %>% 
...
left_join(temp_df, w2_df, by = c("Month"), multiple = "first") -> w2_df 
